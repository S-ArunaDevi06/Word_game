...
try:
    harry_potter_font = pygame.font.Font('HARRYP__.TTF', 20)  # Initial small size for animation
...
try:
    pygame.mixer.music.load('harry_potter_music.mp3')  # Replace with your actual music file
    pygame.mixer.music.play(-1)  # Play the music looped (-1 means infinite loop)
except pygame.error as e:
    print("Error loading music:", e)

# Load background image
try:
    background_image = pygame.image.load('first_background.jpg')  # Your background image file
    background_image = pygame.transform.scale(background_image, (screen_width, screen_height))  # Resize to fit the screen
except pygame.error as e:
    print("Error loading background image:", e)

# Function to apply blur effect using Pillow
def blur_image_with_pillow(pygame_image, radius=1):  # Reduced blur radius
...
text = "Spell Bound"
text_color = (220, 142, 0)  # Golden color
font_size = 30  # Initial font size
max_font_size = 150  # Max font size for the zoom effect
font_size_increment = 2  # How much to increment the font size per frame
...
next_font_size = 30  # Fixed font size for "Next" text

# Game loop
running = True
showing_title_screen = True  # Flag to control the title screen display

while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
            pygame.mixer.music.stop()  # Stop the music when the game closes
    ...
    if showing_title_screen:
        ...
        blurred_background = blur_image_with_pillow(background_image, radius=5)  # Reduced blur radius
        ...
        if font_size < max_font_size:
            font_size += font_size_increment  # Increase font size gradually
        ...
        pygame.draw.rect(screen, (0, 255, 0), (next_button_x, next_button_y, next_button_width, next_button_height))  # Green button
        next_text = next_font.render("Next", True, (255, 255, 255))  # White text on button
        ...
        # After user presses enter, transition to another screen or end the program
        print("Transitioning to next screen...")
        pygame.time.wait(1000)  # Wait for a moment before closing or transitioning
        running = False  # Exit the loop for now (you can change this to another screen later)
